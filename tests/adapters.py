...
def run_rope(
    ...
    """
    return apply_rope(in_query_or_key, token_positions, theta=theta)
...
def run_get_batch(
    ...
    """
    return get_datapoints_from_source(dataset, batch_size, context_length, device)
...
def run_cross_entropy(
    ...
    """
    # Numerically stable: subtract max before log-sum-exp
    shifted = inputs - inputs.max(dim=-1, keepdim=True).values
    log_sum_exp = torch.log(torch.exp(shifted).sum(dim=-1))
    log_probs = shifted[torch.arange(len(targets)), targets] - log_sum_exp
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe(input_path, vocab_size, special_tokens, num_processes = 16)
